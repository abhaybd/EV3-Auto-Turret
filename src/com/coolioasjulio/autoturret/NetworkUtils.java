...
public class NetworkUtils {
     ...
     * 
     * @param index
     *            Index of network interface in
     *            networkInterface.getInterfaceAddresses().get()
     * @return Subnet object representing subnet of the specified network
     *         interface.
     * @throws IOException
     */
    ...
    /**
     * Automatically calculate subnet and continuously send udp packets to every
     * machine on the subnet. Stop as soon as one replies, since that's the
     * server.
     * 
    ...
    /**
     * Send udp packets to every machine on specified subnet. Stop as soon as a
     * machine responds, as that is the server.
     * 
     * @param subnet
     *            Subnet object to scan and send messages to
     * @return InetAddress of server
     * @throws IOException
     */
                                ...
                                for (int d = minIp[3] & 0xFF; d <= (minIp[3] & 0xFF) + range[3]; d++) {
                                    try {
                                        InetAddress addr = InetAddress
                                                .getByAddress(new byte[] { (byte) a, (byte) b, (byte) c, (byte) d });
